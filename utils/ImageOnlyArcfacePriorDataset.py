...
class ImageOnlyArcfacePriorDataset(Dataset):

    '''
    This dataset is a variation of the `CelebADataset`. The difference is that this dataset would create the prior one-hot channel annotation when loading the data. The annotation would be generated with the original InstantID pipeline, so only use this dataset at inference/evaluation time.
    ...
    def __init__(self, data_dir: str, network_input_size: int, arcface_app: FaceAnalysis):
        ...
        self.network_input_size = (network_input_size, network_input_size)
        self.arcface_app = arcface_app

        # Define transform to resize image and convert to tensor
        self.transform = torchvision.transforms.Compose([
            torchvision.transforms.Resize(self.network_input_size, interpolation=torchvision.transforms.InterpolationMode.BICUBIC, antialias=True),
    ...
    def __getitem__(self, idx):
        ...
        image_tensor = torch.cat([image_tensor, affine_mask[None, ...]], dim = 0)
    ...
    def get_affine_mask(self, height: int, width: int, affine_M: torch.Tensor, affine_size: int):
        ...
        return kornia.geometry.transform.resize((mask_x & mask_y).float(), self.network_input_size, interpolation = 'bicubic', antialias = True)